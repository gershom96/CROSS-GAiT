...
class FusionModel(nn.Module):
    def __init__(self, mae_model, causal_cnn_encoder, d_k=160, d_v=160, d_embed=768, num_self_attn_layers=7, num_heads=4, projection_dim=64):
...
class FusionModelWithRegression(nn.Module):
    def __init__(self, fusion_model_path, mae_model, causal_cnn_encoder, d_k=160, d_v=160, d_embed=768, num_self_attn_layers=3, num_heads=4, projection_dim=64, regression_hidden_dim=128):
        super(FusionModelWithRegression, self).__init__()
        self.fusion_model = FusionModel(mae_model, causal_cnn_encoder, d_k=d_k, d_v=d_v, d_embed=d_embed, num_self_attn_layers=num_self_attn_layers, num_heads=num_heads, projection_dim=projection_dim)
